...
function MultipleChoiceFields({ question, options, correctAnswers, onChange, errors }: MultipleChoiceFieldsProps) {
  ...
  function removeOption(index: number) {
    ...
    const newCorrect = correctAnswers.filter((id) => id !== removed.id);
    onChange({ options: updated, correctAnswers: newCorrect });
  }

  function toggleCorrect(index: number) {
    const id = options[index].id;
    const updated = correctAnswers.includes(id)
      ? correctAnswers.filter((ci) => ci !== id)
      : [...correctAnswers, id];
  ...
  const correctError = errors.find(e => e.toLowerCase().includes('correct'));
