// ─── React Topic: TypeScript Interface ───────────────────────────────────────
// An interface describes the "shape" of an object — what properties it has
// and what type each property is.
// By defining Car here we get autocompletion and type-checking everywhere
// the Car type is used, which helps students catch mistakes early.
...
// ─── React Topic: Separating Data from UI ────────────────────────────────────
// Keeping data in its own file makes every component simpler:
//  • Components only worry about HOW to display things
//  • This file only defines WHAT the data is
// Any component that needs the car list just imports it — no duplication.
