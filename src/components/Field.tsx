...
export function Label(props: LabelProps) {
  return <RACLabel {...props} className={twMerge('font-sans text-sm text-black font-medium cursor-default w-fit', props.className)} />;
}
...
export function Input(props: InputProps) {
  return <RACInput {...props} className={composeTailwindRenderProps(props.className, 'px-3 py-0 min-h-9 flex-1 min-w-0 border-0 outline outline-0 bg-neutral-100 dark:bg-neutral-900 font-sans text-sm text-black placeholder:text-neutral-500 dark:placeholder:text-neutral-400 disabled:text-neutral-400 dark:disabled:text-neutral-600 disabled:placeholder:text-neutral-400 dark:disabled:placeholder:text-neutral-600 [-webkit-tap-highlight-color:transparent]')} />
}
